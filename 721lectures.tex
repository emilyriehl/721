...
\isprop(A) &\coloneq \Sigma_{x,y:A} \iscontr(x=y) \\
\is{set}(A) &\coloneq \Sigma_{x,y:A} \isprop(x=y) 
\end{align*}
...
There is a distinction made in mathematics between \emph{properties} and extra \emph{structure}. For instance, given a function $f \colon A \to B$ and term $b : B$, an inhabitant of the type $\fib_f(b) \coloneq \Sigma_{a:A} f(a)=b$ does not merely witness the property that $b$ is in the image of $f$ but also provides the data of an explicit pre-image $a$ and identification $p : f(a) =b$.

To correctly capture the mere property of being in the image, we need a way to assert the \emph{proposition} that a type is inhabited without providing the \emph{data} of a specific inhabitant. The proposition that a type $A$ is inhabited is called the \textbf{propositional truncation} of $A$.
...
\[
\begin{tikzcd} & (A \to Q) \arrow[dl, "- \circ f"'] \arrow[dr, "- \circ f'"] \\ (P \to Q) \arrow[rr, leftrightarrow, "\simeq"] & & (P' \to Q)
\end{tikzcd}
\] 
...
\section*{October 27: The image of a map \& Finite types}
