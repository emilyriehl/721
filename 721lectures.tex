...
\item \textbf{transitive} if it comes with a term $\tau : \Pi_{x,y,z : A} \to R(x,y) \to R(y,z) \to R(x,z)$
\end{itemize}
...
\begin{defn} Let $P \colon \bN \to \UU$. A number $n : \bN$ is a \textbf{lower bound} for $P$ if it comes equipped with a term in the type
  ...
  Similarly,
  \[ \type{is-uppwer-bound}_{P}(n) \coloneq \Pi_{k : \bN} P(k) \to k \leq n\]
  \end{defn}
...
\[ \type{sec}(f) \coloneq \Sigma_{g : B \to A} f\circ g \simeq \id_B.\]
\item The type of \textbf{retractions} of $f$ is defined to be the type
\[ \type{retr}(f) \coloneq \Sigma_{h : B \to A} h \circ f \simeq \id_A.\]
...
\[ \type{has-inverse}(f) \coloneq \Sigma_{g : B \to A} (f \circ g \simeq \id_B) \times (g \circ f \simeq \id_A).\]
...
\[ \term{ind-sing}_a : B(a) \to \Pi_{x:A} B(x) \quad \text{and} \quad \term{comp-sing}_a : \term{ev-pt}\circ\term{ind-sing}_a \simeq \id.\]
\end{defn}
