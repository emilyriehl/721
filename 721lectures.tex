...
\begin{lem} Suppose $A$ and $B$ are types so that $A \leftrightarrows B$. Then $A$ is decidable if and only if $B$ is decidable.
\end{lem}
\begin{proof}
...
\begin{defn} Let $P \colon \bN \to \UU$. A number $n : \bN$ is a \textbf{lower bound} for $P$ if it comes equipped with a term in the type
  ...
  \begin{thm}[well-ordering principle]
    ...
    \[ w(P,d) : \left( \sum_{n : \bN}P(n)\right) \to \left( \sum_{m : \bN}P(m) \times \type{is-lower-bound}(m)\right).\]
  \end{thm}
  ...
  \begin{proof}
    ...
    \[ Q(n) \to \sum_{m : \bN}Q(m) \times \type{is-lower-bound}_Qm
    \]
    ...
    \]
    so we can do a case analysis. If we have $Q(0)$ then it follows immediately that $m=0$ is minimal.  If $\neq Q(0)$, then we can consider the decidable family $Q' \colon \bN \to \UU$ defined by $Q(n) \coloneq Q'(\suc(n))$. Since $q : Q'(n)$ we get a minimal element $m$ for $Q'$ by the inductive hypothesis. But since $Q(0)$ is assumed to be false then $m+1$ is the minimal element for $Q$.   \emd{proof}
...
\begin{thm} For each $n$, there is a prime number $p : \bN$ so that $n < p$.
  ...
  \begin{proof}
    ...
    Since $\neg R(n,x)$ holds we conclude that $\neg(n < x)$. It follows that $x \leq n$.
  \end{proof}
